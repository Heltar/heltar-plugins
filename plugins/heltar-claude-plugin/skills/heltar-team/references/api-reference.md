...
| 400    | `The account for agent@example.com was deleted and its email is still reserved, so it cannot be added here. Contact support if this address needs to be freed.`                              | The email belongs to a deleted account that was never a member of your organisation, so there is nothing here to restore.   |
